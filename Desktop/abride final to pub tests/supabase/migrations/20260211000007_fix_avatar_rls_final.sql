-- إصلاح نهائي لـ RLS policies للصور (بدون تعديل الجدول مباشرة)
-- هذا الملف يحل مشكلة "new row violates row-level security policy" بشكل نهائي
-- لا نحتاج إلى ALTER TABLE لأن RLS مفعّل افتراضياً على storage.objects
...
-- 2. حذف جميع السياسات القديمة للصور (إذا كانت موجودة)
